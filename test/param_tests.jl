...
xn = [Normal(log(1), 0.5) for _ in 1:T-1]
push!(xn, Normal(log(1), 0.5))
q = MeanFieldGaussian(xn)

# Run ADVI algorithm
elbo = ELBO(1)
advi = ADVI(5, 300, 2, 100, 1e-2, VarInfLogger(N, L))
@time Q = optimize(advi, elbo, q, bm, data, 0.1, 0.07)
...
# init!(ch)
# rjmcmc!(ch, 2000, show=10, trace=1)
...
all_rates = [Symbol(string("μ", i)) for i in 1:L]
# append!(all_rates, [Symbol(string("μ", i)) for i in 1:L])
compare_methods(Q, bm, advi.logger, ch, all_rates, x, 500)
