...
p = params(q)
init = [p.μ[1:(2*K)]..., repeat([0.5], 200)..., 0.1 .* ones(2K + 2N)...]
...
advi = ADVI(1, 1000, 2, 10, logger)
res, index = advi(elbo, q, model, data, 1., .8)
θ = params(res)[:μ]

# Convert to correct probabilities
ϕ = model_transform(model)(θ)
ϕ = ϕ.ϕ
for i in 1:N
    ϕ[i,:] ./= sum(ϕ[i,:])
end

function count_class1(ϕ)
    c = 0
    for i in 1:N
        c += (argmax(ϕ[i, :])[1]-1)
    end
...
count_class1(ϕ)
println(ϕ)
a = ϕ_true .== get_results(ϕ)
i = [i for i in Iterators.flatten(a)]
acc()
